...
export default function TxClient({ initial }: { initial: AdminTx[] }) {
  ...
  return (
    <div className="rounded-3xl border border-[#4a6c7a]/45 bg-[#fbf3e2]/95 p-5 md:p-7">
      <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
        <h2 className="text-xl font-semibold">Giao dịch ({filtered.length})</h2>
        <div className="flex gap-2">
          {(['pending', 'completed', 'all'] as const).map((f) => (
            ...
            >
              {f === 'pending' ? 'Chờ duyệt' : f === 'completed' ? 'Đã duyệt' : 'Tất cả'}
            </button>
          ))}
        </div>
      </div>
