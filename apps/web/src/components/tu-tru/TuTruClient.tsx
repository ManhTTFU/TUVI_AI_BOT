...
function TruIntroCards() {
  const ROWS = [
    { name: 'Năm Trụ', kanji: '年柱', desc: 'Tổ tiên, ông bà, năm sinh và gốc rễ vận mệnh.' },
    { name: 'Tháng Trụ', kanji: '月柱', desc: 'Cha mẹ, thời niên thiếu, môi trường lớn lên.' },
    { name: 'Ngày Trụ', kanji: '日柱', desc: 'Bản thân (Nhật chủ) + người bạn đời (Chi ngày).' },
    { name: 'Giờ Trụ', kanji: '時柱', desc: 'Con cái, hậu vận, sự nghiệp cuối đời.' },
  ];
  return (
    <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {ROWS.map((r) => (
        ...
        >
          <div className="text-[14px] font-serif text-[#5a3a1a]" style={{ fontFamily: SERIF_FONT }}>
            {r.name}
          </div>
          <div className="text-[11px] tracking-[0.2em] text-[#4a6c7a] mt-0.5">
            {r.kanji}
          </div>
          <p className="mt-3 text-[12.5px] text-[#0f0a08] leading-relaxed">{r.desc}</p>
        </div>
      ))}
    </section>
  );
...
export default function TuTruClient() {
  ...
  const [chartMeta, setChartMeta] = useState<{ name: string; gender: Gender } | null>(null);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Đã lập Tứ Trụ Bát Tự — đang chờ AI luận giải');
    } catch (err) {
    ...
    <div className="relative text-[#0f0a08]">
      ...
      <section className="pt-12 pb-8 px-6 text-center">
        ...
        <p className="mt-4 max-w-2xl mx-auto text-[#4a3a30]">
          Lập 4 trụ Năm – Tháng – Ngày – Giờ theo Can Chi. Luận giải Nhật chủ, dụng
          thần, kỵ thần, ngũ hành nạp âm, sự nghiệp, tài lộc, tình duyên qua AI.
        </p>
      </section>
      ...
      <section className="px-6">
        <div className="max-w-2xl mx-auto rounded-3xl border border-[#4a6c7a]/45 bg-[#fbf3e2]/95 p-6 md:p-9 shadow-[0_30px_80px_-30px_rgba(90,58,26,0.3)]">
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <div>
              <FieldLabel icon="📅">Ngày / Tháng / Năm sinh & Giờ sinh (dương lịch)</FieldLabel>
            ...
            {proRequired && (
              <div className="rounded-xl border border-[#c89146]/55 bg-[#f5e3c0]/50 px-4 py-3 text-[#5a3a1a] text-[13px] space-y-2">
                <div>
                  ⚠ <strong>Cần tài khoản PRO.</strong> Đăng ký gói (từ 20.000đ/tháng) để xem
                  Tứ Trụ Bát Tự không giới hạn.
                </div>
                <Link
                  href="/vi-cua-toi"
